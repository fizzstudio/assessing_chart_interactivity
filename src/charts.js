...
const heatClick = (d) => {
    d.target.clickHighlight = d.target.clickHighlight && d.target.clickHighlight[0] === d.detail.data ? [] : [d.detail.data]
    document.getElementById("aria-live").innerHTML = !d.target.clickHighlight.length ? 'Selected data has been removed.' : d.detail.data.Value > .95 ? 'Success: Highest approval correlation has been selected.' : `${d.detail.data.Note} has been selected.`
}
const heatHover = (d) => {
...
const props = {
    'bar-chart': {
        mainTitle:
          `Products in all Regions, 2020`,
        subTitle: "This chart shows all products sales across every region, sorted from worst-performing to best. Values are in millions of dollars.",
        data: years["2020"],
    ...
    'line-chart': {
        mainTitle:
          'Sales in Related Peer Products "A" and "B"',
    ...
    'stacked-bar-chart': {
        mainTitle:
          "All Products Trending Down in 2022",
        subTitle:
    ...
    'heat-map': {
        mainTitle:
          "Comparing how user groups correlate to approval rates of product types",
        subTitle: "A high correlation value (close to 1) indicates that these users have a high approval rate of that product. A low correlation value (close to -1) indicates a negative approval rate of those products.",
        data: [
          { Product: "A", "User group": "Orders high takeout", Value: .74, Note: "High approval correlation" },
          { Product: "B", "User group": "Orders high takeout", Value: .43, Note: "Medium approval correlation" },
          ...
          { Product: "B", "User group": "Midwest Rural", Value: .09, Note: "Low approval correlation" },
          ...
          { Product: "C", "User group": "MSA > 200k", Value: .97, Note: "High approval correlation" },
          { Product: "D", "User group": "MSA > 200k", Value: -0.76, Note: "Very low approval correlation" },
          { Product: "A", "User group": "Travelled to France", Value: -.9, Note: "Lowest approval correlation" },
          ...
          { Product: "D", "User group": "Travelled to France", Value: .02, Note: "Low approval correlation" }
...
const swapCharts = (e) => {
    ...
    document.getElementById(`chart${chartNumber}`)
        .querySelector('.insert').appendChild(chart)
}
